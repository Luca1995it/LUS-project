...
\title{Project of the Natural Language Processing Course - Luca Di Liello}
...
\begin{abstract}
This document contains the instructions to prepare, implement and test a simple 
natural language understanding system, who has to classify each word of a sentence 
to the corresponding IOB notation. The program has been developed using the open-source 
libraries OpenFST and OpenGRM, which allows the creation and the manipulation of FSA 
(finite state acceptors) and FST (finite state transducers).    
\end{abstract}

\section{Credits}

The software showed in this document implements the techniques learned
from the LUS course by professor Giuseppe Riccardi and professor Evgeny Stephanov.
The core of the software is powered by the open-source libraries OpenFST and 
OpenGRM which allow the manipulation of FSA and FST structures.

\section{Introduction}

One of the most important (and difficult) task in LUS systems is the extraction of different 
concepts from a given sentence. There are lots of different notations that has been created
to describe concepts, and one of that is called IOB. The IOB notation (short for Inside, Outside, Beginning)
is often used to tag different tokens of a sentence with a corresponding O,
B or I tag, basically creating a map from words to concepts. 
O tags are used to specify words that do not belong to a specific 
concept (e.g. stopwords), B tags are words that specify the start of 
a new concept while I tags follow B tags in concepts composed by
many word. In our case, we will use a dataset containing sentences 
and the corresponding IOB tags which will refer to entities about cinema and movies.
In addition, for each sentence, we will have at our disposal the equivalent POS tagging
and lemma list.
The whole program will be sustained by the already mentioned OpenFST and OpenGRM libraries,
which can be used both from the command line and from C/C++ code. Our implementation and results will
refer to Bash and Python scripts, but implementing same actions in C/C++ will lead to similar (or probably better)
time performances.
These libraries allow programmers to play with complex weighted finite state machines without having to write code
to do some basic operations like union and intersection and epsilon removal. Moreover, they offer more complex
functions like composition, determinization and minimization.
Finally, the evaluation of the written software tuned by different parameters is done principally
using the F1 score but also taking into account the time needed by the algorithm to complete.
